...
folder = cwd[:-22]+'Data\\VapourTemperature\\2023-12-01-CellTemperature\\'

filename = '20231201-OD-74oC.csv'
...
plt.plot(time_data,y_data_satspec)
plt.show()
...
#fit polynomial to background data (away from dips) to account for slope for reference cell
...
#fit polynomial to background data (away from dips) to account for slope for warm cell
p = np.polyfit(f_array[indices_spec], y_data_cell[indices_spec], 2)
bg = np.polyval(p, f_array)
...
plt.plot(f_array[indices_spec], y_data_cell[indices_spec],'o') #indices used in background calculation
...
temp_init = 70
temp_Dopp = 50
Constrain = True # choose true to have the temp and Doppler temp constrained, false to have as independent parameters
