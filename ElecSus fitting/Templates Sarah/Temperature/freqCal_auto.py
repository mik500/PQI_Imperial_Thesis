...
def readDataICL(file, smooth):
    wb = pd.read_csv(file, skiprows= 2, names = ['time', 'ch1', 'ch2'])# xlrd.open_workbook(file)
    #sheet = wb.sheet_by_index(0)
    y_data_ch1 = np.asarray(wb['ch1'])
    y_data_ch3 = np.asarray(wb['ch2'])
...
def normaliseSatSpecData(time_data,y_data):# noramlises the data such that off-resonance is a transmision of 1 i.e. OD of 0. Returns OD
    # defining the indices for background
    OD = -np.log(y_data/max(y_data))
    peak_points, _ = find_peaks(OD, prominence=0.08, distance=50)
    print(peak_points)
    plt.plot(OD)
...
def frequencyCalibrate(time_data,chan1,chan3, RbFrac, cellLength, temp, temp_Dopp, plotFlag, cellFlag, Constrain):
    un_pumped_satSpec, f_array, peak_points = normaliseSatSpecData(time_data,chan3)
    ...
    p0=[0, 1, -1e-3,1e-6, temp, temp_Dopp, RbFrac]
    ET.globalVars(cellLength,Constrain) # send through the variables 
    [freq, residuals, temperature, temp_Doppler, Rb85Frac, popt] , [data_signal,_fit,residual, res_max]= ET.extractTemp(f_array, _signal, p0, plotFlag)
    
    return [freq, residuals, temperature, temp_Doppler, Rb85Frac, popt], [data_signal,_fit,residual, res_max]
