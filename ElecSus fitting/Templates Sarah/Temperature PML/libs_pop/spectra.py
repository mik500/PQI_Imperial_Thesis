...
p_dict_defaults = {	'Elem':'Rb', 'Dline':'D2', 
							...
							'BoltzmannFactor':True, 'popShift':0}
...
def FreqStren(groundLevels,excitedLevels,groundDim,
	...
	# Ground state population vector for Rubidium
 
	# Hyperfine degenracies
	gF1 = 3
	gF2 = 5
	gF3 = 7
	...
	if groundPop == 0:
		...
		if Rb_isotope == '85': 
			...
			groundPopArray[0:gF2-1] = groundPopArray[0:gF2-1]+popShift/gF2 # Address F=2 population
			groundPopArray[gF2:] = groundPopArray[gF2:]-popShift/gF3  # Address F=3 population
		...
		if Rb_isotope == '87':
			...
			groundPopArray[0:gF1-1] = groundPopArray[0:gF1-1]+popShift/gF1  # Address F=1 population
			groundPopArray[gF1:] = groundPopArray[gF1:]-popShift/gF2  # Address F=2 population


	if groundPop == 1:
		groundPopArray = np.ones(groundDim)
		groundPopArray[groundPop:] = 0
		groundPopArray[:groundPop] = 0
		groundPopArray[groundPop] = groundDim
	...
	for gg in range(groundDim):
		for ee in interatorList:
			# Calcualte Clebsch-Gordan coefficients
			...
			if cleb2 > 0.0005: #If negligable don't calculate.
				# Transition freq. is the difference of ground and exicted state
...
def get_spectra(X, E_in, p_dict, groundPop, popShift, outputs=None):
	""" 
	...
	ChiPlus, ChiMinus, ChiZ = calc_chi(X, p_dict, groundPop, popShift)
